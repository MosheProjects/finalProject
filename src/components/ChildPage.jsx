...
export default function ChildPage() {
  ...
  const [ curAgeFields, setCurAgeFields ] = useState();
  const [ curVaccineKey, setCurVaccineKey ] = useState(1);
  ...
  return (
    <div dir="rtl" className="bg-peach p-4">
      ...
      {(loadingVaccines) ? null :
        (<div className="  p-1 border border-1 rounded">
          <p className="text-center fs-3 m-2 border-bottom">תוכנית החיסונים בטיפת חלב</p>
                ...
                <Tab key={i} eventKey={item.month} title={item.name}>
                  <ListGroup className="bg-peach container" as="ol" numbered>
                    {item.vaccines.map((vac, i) => {
                      return (<ListGroup.Item className="container" as="li">
