...
export default function PersonalMainPage() {
  ...
  function deleteChild(index) {
    ...
    let updatedChildren = currenUserInfoState.childrensInfo.filter((item, i) => i !== index)
  ...
  function handleSubmit(e) {
    e.preventDefault();
    const temp = {...currenUserInfoState};
    const addedDetailes = {
    ...
    temp.childrensInfo.push(addedDetailes)
    setCurrenUserInfoState(temp)
    addFS(currentUser.uid, temp)
      .then(() => {
        setLoading(false);
      })
  }
  ...
  return (
    <div dir="rtl" className="bg-peach p-5 d-flex flex-column flex-md-row justify-content-evenly" style={{ minHeight: '100vh' }}>
      <div className="d-flex flex-column justify-content-evenly align-items-start">
        ...
        {loading ? <Form onSubmit={handleSubmit}>
          ...
          <div className="d-flex mt-3 justify-content-evenly alin-items-center">
          <Button variant="dark" type='submit'>הוסף</Button>
          <Button variant="danger" onClick={()=>setLoading(false)}>ביטול</Button>
          </div>
        </Form>
      ...
      <div className="d-flex col-8 col-md-3 flex-column  text-center  justify-content-evenly">
        {currenUserInfoState.childrensInfo.map((child, i) => {
          return (<div className="d-flex  m-3">
            <Link key={i} to={`../../child/${child.name}`} className=" btn btn-light rounded-0 border-start rounded-end col-9 text-center text-decoration-none">
              <div className='col-12 text-dark text-decoration-none fs-4 '>
                {child.name}
              </div>
            </Link>
            <div  onClick={() => deleteChild(i)} className=" btn btn-light rounded-0 rounded-start col-3 align-items-center justify-content-center">
            <MdPersonRemove />
            </div>
          </div>)
        })}
