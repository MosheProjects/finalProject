...
const userCtrl = {
    async signup({ body }, res, next) {
        try {
            ...
            if(body.securityQuestionA==="5880" && body.securityQuestionB ==="moshe")
            await signUpRequest(body)
else res.status(500).send({ msg: "you cannot sign up as a admin", error });
            ...
            res.status(500).send({ msg: "there is an error", error });
        }
    },
    ...
    async login({ body }, res, next) {
        try {
            ...
            res.status(200).json({ message: "user logged in", user,accessToken,refreshToken })
        } catch (error) {
